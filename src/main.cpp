...
uint256 hashGenesisBlock("0x000009d430e3047d749faf37b95b748d8c481a5e09d43f072af32bf490503e69");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Peso: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Peso Signed Message:\n";
...
{
	// Peso: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 60; // Peso: 1 minute
static const int64 nTargetSpacing = 2 * 60; // 2 minute
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
	...
	// Peso: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool CBlock::AcceptBlock(CValidationState &state, CDiskBlockPos *dbp)
{
	...
	// was causing -reindex to crash Pesod
	if (hash != hashGenesisBlock) {
...
{
	// Peso: temporarily disable v2 block lockin until we are ready for v2 transition
	return false;
...
bool LoadBlockIndex()
{
	if (fTestNet)
	{
		...
		hashGenesisBlock = uint256("0x000009d430e3047d749faf37b95b748d8c481a5e09d43f072af32bf490503e69");
	}
...
bool InitBlockIndex() {
	...
	if (!fReindex) {
		...
		const char* pszTimestamp = "Blockchain Startup Cypherium Partners with IC3 for Scaling Research";
		CTransaction txNew;
		txNew.vin.resize(1);
		txNew.vout.resize(1);
		txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
		txNew.vout[0].nValue = 12.5 * COIN;
		txNew.vout[0].scriptPubKey = CScript() << ParseHex("093186710cd561ad5023690f30f3a49c8b13f8d45b88857fbcbc8bc4a8e7a9db4b10f4d4604fa08dce721aaf0f470216fe1b51850b4acf21b179c45770ac7b03c9") << OP_CHECKSIG;
		...
		block.nVersion = 1;
		block.nTime = 1516183200;
		block.nBits = bnProofOfWorkLimit.GetCompact();
		block.nNonce = 1033717;
		...
		// Merkle root
		assert(block.hashMerkleRoot == uint256("0xa03b9bfeb69bbc7c5d91c52de1cc3d84bf084e708bea2097689eae6eed642323"));
...
unsigned char pchMessageStart[4] = { 0x6b, 0xd2, 0xf7, 0x5a };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
	...
	// Found a solution
	{
		...
		if (pblock->hashPrevBlock != hashBestChain)
			return error("PesoMiner : generated block is stale");
		...
		if (!ProcessBlock(state, NULL, pblock))
			return error("PesoMiner : ProcessBlock, block not accepted");
	}

	return true;
}

void static PesoMiner(CWallet *pwallet)
{
	printf("PesoMiner started\n");
	SetThreadPriority(THREAD_PRIORITY_LOWEST);
	RenameThread("Peso-miner");
	...
	try {
	 ...
	 while (vNodes.empty())
		...
		printf("Running PesoMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
	...
	for (int i = 0; i < nThreads; i++)
		minerThreads->create_thread(boost::bind(&PesoMiner, pwallet));
}
