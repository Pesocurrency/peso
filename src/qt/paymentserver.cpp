...
const int Peso_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString Peso_IPC_PREFIX("Peso:");
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    {
        if (!args[i].startsWith(Peso_IPC_PREFIX, Qt::CaseInsensitive))
            continue;
    ...
    foreach (const QString& arg, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(Peso_IPC_CONNECT_TIMEOUT))
            return false;
        ...
        socket->waitForBytesWritten(Peso_IPC_CONNECT_TIMEOUT);
...
PaymentServer::PaymentServer(QApplication* parent) : QObject(parent), saveURIs(true)
{
    ...
    if (!uriServer->listen(name))
        qDebug() << tr("Cannot start Peso: click-to-pay handler");
    else
